...
import { ExploreDungeonUseCasePlayerService } from 'App/Core/use-cases/explore-dungeon.use-case'
...
export interface PlayerServiceExploreResultRepository {
    ...
    ExploreDungeonUseCasePlayerService
{
  constructor(
    ...
    private readonly exploreResultRepository: PlayerServiceExploreResultRepository
  ) {}
  ...
  public async create(name: string, password: string): Promise<Player> {
    ...
    player = new Player(name, new Backpack())
  ...
  public explore(player: Player, dungeon: Dungeon): Promise<Report> {
    const score = player.explore(dungeon)
    const message = dungeon.events.map((property) => property.description).join('\n')
    return this.exploreResultRepository.save(new Report(player, dungeon, score, message))
  }
}
